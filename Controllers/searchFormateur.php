...
foreach ($res as $row) {
    ...
    if ($a->RechAdmin($row[3])->fetchColumn(0) == 0) {
        $result = $result . '<td class="align-middle text-center text-sm"><a href ="../../../Controllers/makeAdminFormateur.php?email=' . $row[3] . '">Make Admin</a></td>';
    } else {
    ...
    $result = $result . '<td class="align-middle text-center text-sm">
        ...
        <a  class="text-secondary font-weight-bold text-xs" data-toggle="tooltip" data-original-title="Edit user" href="../../../Controllers/deleteFormateur.php?idFormateur='.$row[0].'">
            ❌
        </a>
            </td>
        </tr>
...
$_SESSION["res"] = $result;
header("location:../Views/admin/pages/tableFormateur.php");
