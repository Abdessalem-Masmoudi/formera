							...
							<div class="header_content d-flex flex-row align-items-center justify-content-start">
								...
								<nav class="main_nav">
									<ul>
										...
										<?php
										if ((isset($_SESSION["passwordUser"]) && isset($_SESSION["emailUser"])) || (isset($_SESSION["password"]) && isset($_SESSION["email"])) || (isset($_SESSION["passwordAdmin"]) && isset($_SESSION["emailAdmin"]))) {
			...
			<div class="container" style='padding-top:70px'>
				...
				<div class='col-lg-6'>
					<div class='details_image'>
						<div class='details_image_large' ><img src='images\\formation\\" . $row[5] . "' alt='' height='400px' width='auto'><div></div></div>
					</div>
				</div>
				...
				<div class='col-lg-6'>
					<div class='details_content'>
						...
						<div class='details_name'>$row[1]</div>
						<div class='details_price' style='font-size: 20px;'>$row[2] TND</div>
					...
					if ((isset($_SESSION["passwordUser"]) && isset($_SESSION["emailUser"]))) {
						if (!$part->verifPartnUserForm($_SESSION["idUser"], $row[0])->fetch()["count(*)"] != 0) {
							echo "
					...
					if($rowRate[3]==-1)
						...
						if (!$fav->verifFavouritUserForm($_SESSION["idUser"], $row[0])->fetch()["count(*)"] != 0) {
							...
							echo "<button class='btn btn-lg'><a href= '../../Controllers/Annulerfavourit.php?idFormation=" . $row[0] . "'  >💔retirer </a></button>";
						}
					} else {
						if ((isset($_SESSION["passwordAdmin"]) && isset($_SESSION["emailAdmin"]))) {
							echo "
							<button class='btn btn-lg'><a>Login as User please !</a></button>
							";
						} elseif (isset($_SESSION["password"]) && isset($_SESSION["email"])) {
							echo "<button class='btn btn-lg'><a href= 'EditFormation.php?idFormation=" . $row[0] . "'  >📝Edit </a></button>";
							echo "<button class='btn btn-lg'><a href= '../../Controllers/DeleteFormation.php?idFormation=" . $row[0] . "'  >❌Delete </a></button>";
						} else {
							echo "
							<button class='btn btn-lg'><a href= 'login.php'>Login</a></button>
							";
						}
					}
					?>
